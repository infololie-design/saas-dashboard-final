...
import { 
  ...
  Menu, X, Mail, FileBarChart, Wallet
} from 'lucide-react';
...
const Dashboard = ({ session }) => {
  ...
  const cargoData = useMemo(() => {
    ...
    const filtered = data.my_results.filter(item => 
      ...
      (item.content && item.content.toLowerCase().includes(cargoFilter.toLowerCase()))
    );
  ...
  const financeSummary = useMemo(() => {
     if (activeTab !== 'finance' || !data || !Array.isArray(data)) return { totalMissing: 0, count: 0 };
     const missingItems = data.filter(i => i.mutabakat_durumu.includes('EKSİK'));
     const total = missingItems.reduce((acc, i) => acc + (Number(i.fark) || 0), 0);
     return { totalMissing: total, count: missingItems.length };
  }, [data, activeTab]);
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      // --- HATA YÖNETİMİ (DÜZELTİLDİ: Dizi İçindeki Hatayı da Yakalar) ---
      const isError = !response.ok || 
                      result.status === 'error' || 
                      result.status === 'duplicate' ||
                      result.error || 
                      (Array.isArray(result) && result[0] && (result[0].status === 'error' || result[0].error));

      if (isError) {
         let errorMsg = result.message || result.error;
         
         // Dizi içinden hata mesajını çıkar
         if (Array.isArray(result) && result[0]) {
             errorMsg = result[0].message || result[0].error || "Bilinmeyen hata";
         }
         
         if (errorMsg && (String(errorMsg).includes('duplicate') || String(errorMsg).includes('zaten kayıtlı'))) {
             alert("⚠️ DİKKAT: Bu fatura numarası zaten sistemde kayıtlı!");
         } else {
             alert("İşlem başarısız: " + (errorMsg || "Hata"));
         }
         setLoading(false);
         return; 
      }
  ...
  const handlePurchaseUpload = async (e) => {
    ...
    try {
       ...
       const response = await fetch('/api/proxy', {
           method: 'POST',
           headers: { 'Content-Type': 'application/json' },
           body: JSON.stringify({ targetUrl: 'https://n8n.lolie.com.tr/webhook/bulk-purchase-upload', user_id: session.user.id, email: session.user.email, file_data: base64File, file_name: file.name })
       });
       const result = await response.json();
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  {activeTab === 'cost' && (<><input type="file" id="purchaseInput" className="hidden" accept=".xlsx,.xls" onChange={handlePurchaseUpload} disabled={loading} /><label htmlFor="purchaseInput" className={`cursor-pointer flex justify-center items-center gap-2 px-4 py-2 rounded-lg border border-dashed border-gray-300 hover:border-green-500 hover:bg-green-50 hover:text-green-700 transition w-full sm:w-auto`}><Upload size={16} /> Alım Faturası Yükle</label></>)}
                   ...
                   <div className="space-y-6">
                      <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                          <div className="flex items-center gap-4 w-full md:w-auto">
                              <div className={`p-3 rounded-full ${financeSummary.totalMissing > 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                                  {financeSummary.totalMissing > 0 ? <AlertTriangle size={24}/> : <CheckCircle size={24}/>}
                              </div>
                              <div>
                                  <p className="text-xs text-gray-500 uppercase font-bold">{financeSummary.totalMissing > 0 ? 'Eksik Yatan Tutar' : 'Tüm Ödemeler Tam'}</p>
                                  <h3 className={`text-2xl font-bold ${financeSummary.totalMissing > 0 ? 'text-red-600' : 'text-green-600'}`}>{formatCurrency(financeSummary.totalMissing)}</h3>
                                  {financeSummary.totalMissing > 0 && (<p className="text-xs text-gray-400">{financeSummary.count} siparişte eksik tespit edildi.</p>)}
                              </div>
                          </div>
                          <div className="relative w-full md:w-64"><Search className="absolute left-3 top-2.5 text-gray-400" size={18} /><input type="text" placeholder="Ara..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" value={financeFilter} onChange={(e) => setFinanceFilter(e.target.value)} /></div>
                      </div>
                      <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6"><div className="overflow-x-auto"><table className="w-full text-left min-w-[700px]"><thead className="bg-gray-50 text-gray-600 text-xs uppercase font-semibold"><tr><th className="p-4">SİPARİŞ</th><th className="p-4">TARİH</th><th className="p-4 text-right">BEKLENEN</th><th className="p-4 text-right">YATAN</th><th className="p-4 text-center">DURUM</th></tr></thead><tbody className="divide-y divide-gray-100 text-sm">{financeData.map((row, i) => { let statusColor = 'bg-gray-100 text-gray-600'; if (row.mutabakat_durumu === 'TAMAMLANDI') statusColor = 'bg-green-100 text-green-700'; else if (row.mutabakat_durumu.includes('EKSİK')) statusColor = 'bg-red-100 text-red-700'; else if (row.mutabakat_durumu.includes('FAZLA')) statusColor = 'bg-blue-100 text-blue-700'; return (<tr key={i} className="hover:bg-gray-50/50"><td className="p-4 font-medium">{row.order_number}</td><td className="p-4 text-gray-500">{new Date(row.order_date).toLocaleDateString('tr-TR')}</td><td className="p-4 text-right">{formatCurrency(row.beklenen_tutar)}</td><td className="p-4 text-right font-bold">{formatCurrency(row.hakedis_tutari)}</td><td className="p-4 text-center"><span className={`px-3 py-1 rounded-full text-xs font-bold ${statusColor}`}>{row.mutabakat_durumu}</span></td></tr>); })}</tbody></table></div>{financeData.length === 0 && <div className="text-center p-8 text-gray-400">Veri bulunamadı.</div>}</div>
                   </div>
                )}
                ...
                {activeTab === 'cargo' && (
                   <div className="space-y-6">
                      <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                          ...
                          <div className="relative w-full md:w-64"><Search className="absolute left-3 top-2.5 text-gray-400" size={18} /><input type="text" placeholder="Sipariş veya İçerik ara..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none" value={cargoFilter} onChange={(e) => setCargoFilter(e.target.value)} /></div>
                      </div>
...
const MenuButton = ({ id, icon, label, activeTab, setActiveTab }) => (<button onClick={() => setActiveTab(id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition border-b md:border-b-0 border-gray-50 ${activeTab === id ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'}`}>{icon} {label} {activeTab === id && <ChevronRight size={16} className="ml-auto" />}</button>);
const StatCard = ({ label, value, color = 'gray', isCurrency = true }) => (<div className={`bg-white p-4 rounded-lg shadow-sm border border-${color}-100`}><p className="text-xs text-gray-500 uppercase">{label}</p><p className={`text-lg font-bold text-${color}-600 mt-1`}>{typeof value === 'number' && isCurrency ? formatCurrency(value) : value}</p></div>);
...
const SimpleTable = ({ headers, rows, keys }) => (<div className="overflow-x-auto"><table className="w-full text-left min-w-[500px]"><thead className="bg-gray-50 text-gray-600 text-xs uppercase font-semibold"><tr>{headers.map((h, i) => <th key={i} className="p-4">{h}</th>)}</tr></thead><tbody className="divide-y divide-gray-100 text-sm">{rows?.map((row, i) => (<tr key={i} className="hover:bg-gray-50/50">{keys.map((k, j) => {
  ...
  if (k === 'usd_bagli_para' && row[k]) { val = <FormatDualCurrency tl={row['bagli_para']} usd={row[k]} />; }
  else if (isNumber) { 
      if (isPlainNumber) {
